...
public class KZMSGQOO_Object extends VmlObjectOperations
{
   ...
   FindErrorFields( )
   {
      ...
      // int nRC = SetCursorFirstEntityByInteger( vMsgQ, "Task", "Id", task.getTask( ).getTaskId( ), "" );
         int nRC = vMsgQ.cursor( "Task" ).setFirst( "Id", task.getTask( ).getTaskId( ) ).toInt( );
         if ( nRC == 0 )
         {
            nRC = vMsgQ.cursor( "WebMsg" ).setFirst( ).toInt( );
         // TraceLineI( "FindErrorFields SetCursorFirst WebMsg RC: ", nRC );
            ...
            while ( nRC == 0 )
            {
               ...
               s = GetStringFromAttribute( vMsgQ, "WebMsg", "ErrorMapValue" );
               sbError.append( "\t\n" );
